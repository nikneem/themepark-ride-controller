using ThemePark.Rides.Api._Shared;
using ThemePark.Shared.Enums;

namespace ThemePark.Rides.Api.StartRide;

/// <summary>
/// Transitions a ride from <see cref="RideStatus.Idle"/> to <see cref="RideStatus.Running"/>.
/// Returns 404 if not found, 409 if not Idle.
/// </summary>
public sealed class StartRideHandler(IRideStateStore store)
{
    public async Task<IResult> HandleAsync(string rideId, CancellationToken cancellationToken = default)
    {
        var state = await store.GetAsync(rideId, cancellationToken);
        if (state is null)
            return Results.NotFound();

        if (state.OperationalStatus != RideStatus.Idle)
            return Results.Conflict(new { error = $"Ride is in status '{state.OperationalStatus}'; expected Idle." });

        await store.SaveAsync(state.WithStatus(RideStatus.Running), cancellationToken);
        return Results.Ok();
    }
}
